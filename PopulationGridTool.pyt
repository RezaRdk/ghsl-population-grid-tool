...
class RasterToPopulationGrid(object):
    ...
    def updateParameters(self, parameters):
        ...
        current_base = os.path.basename(current_value) if current_value else None

        # The very first time this runs, ArcGIS Pro may have already filled
        # in its own auto-generated default name (e.g. based on the tool's
        # name) before our code ever touched it. Remember that as a second
        # "still default" baseline, in addition to whatever we ourselves
        # suggested last, so we know it's still safe to overwrite.
        if self._baseline_name is None:
            self._baseline_name = current_base

        is_still_default = (
            current_base is None
            or current_base == self._last_suggested_name
            or current_base == self._baseline_name
        )

        if is_still_default:
            current_dir = os.path.dirname(current_value) if current_value else None
                    ...
                    for chunk in r.iter_content(chunk_size=1024 * 1024):
                        ...
                        if total_size:
                            ...
                            if pct != last_reported_pct and pct % 5 == 0:
                                messages.addMessage(
                                    "Downloading... {0}% ({1:.1f} / {2:.1f} MB)".format(
                                        pct, downloaded / 1e6, total_size / 1e6
                                    )
                                )
                                arcpy.SetProgressorLabel(
                                    "Downloading GHS-POP raster ({0}%)...".format(pct)
                                )
                                last_reported_pct = pct
                        elif downloaded % (50 * 1024 * 1024) < len(chunk):
                            messages.addMessage(
                                "Downloaded {0:.1f} MB so far...".format(downloaded / 1e6)
                            )
            messages.addMessage("Download complete ({0:.1f} MB).".format(downloaded / 1e6))
        else:
